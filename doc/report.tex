...
To test this, I aggregated the data by Source IP (ip.src) to calculate both the total flow count and the total bytes transmitted per host. By comparing the top 10 most active IPs by flow count against the top 10 IPs by data volume, we can isolate the scanner. Any IP that dominates the flow count but is entirely absent from the top data exchangers is confirmed to be exhibiting scanning behavior.

According to the plots shown below, the suspicious IP is \textbf{195.236.242.78}. It starts the majority of flows (\cref{fig:top10-ip-flowcount}), but it does not appear in the top $10$ data movers (\cref{fig:top10-ip-data}).
